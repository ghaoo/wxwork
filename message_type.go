...
type VideoMessage struct {
	...
	Description string `json:"description"` // 视频介绍
	MediaMessage
}
...
type TextCardMessage struct {
	...
	Url         string `json:"url"`         // 消息跳转链接
	BtnTxt      string `json:"btntxt"`      // 按钮文字, 默认为“详情”
}
...
type NewsMessage struct {
	Articles []NewsArticle `json:"articles"` // 图文消息内容，支持1-8条图文
}

// NewsArticle 图文消息内容
type NewsArticle struct {
	Title       string `json:"title"`       // 图文标题
	Description string `json:"description"` // 图文描述
...
type MPNewsMessage struct {
	Articles []MPNewsArticle `json:"articles"` // 图文消息内容，支持1-8条图文
}

// MPNewsArticle 图文消息内容（mpnews）
type MPNewsArticle struct {
	Title            string `json:"title"`              // 图文标题
	ThumbMediaId     string `json:"thumb_media_id"`     // 缩略图素材ID
	Author           string `json:"author"`             // 作者
	ContentSourceUrl string `json:"content_source_url"` // 页面链接
	Content          string `json:"content"`            // 消息内容
	Digest           string `json:"digest"`             // 消息描述
}
...
type MiniprogramNoticeMessage struct {
	...
	Page              string            `json:"page"`                // 点击消息卡片后的小程序页面，仅限本小程序内的页面
	Title             string            `json:"title"`               // 消息标题
	Description       string            `json:"description"`         // 消息描述
	EmphasisFirstItem bool              `json:"emphasis_first_item"` // 是否放大第一个content_item
	ContentItem       map[string]string `json:"content_item"`        // 消息内容键值对，最多允许10个item
}
...
type TaskCardMessage struct {
	...
	Btn         []TaskCardBtn `json:"btn"`         // 按钮列表，按钮个数为为1~2个
}
...
type TaskCardBtn struct {
	...
	ReplaceName string `json:"replace_name"` // 点击按钮后显示的名称
	Color       string `json:"color"`        // 按钮字体颜色
	IsBold      bool   `json:"is_bold"`      // 按钮字体是否加粗
}
